...
- **Record decoding:** AOT Static Compilation (CBOR → Static Structs → Arrow IPC). Incoming CBOR bytes are mapped directly into statically compiled Rust structs via `serde` and `arrow`.
...
1. **gRPC Server:** Tonic serves the `Ingest` (bidi-streaming, ACK per record), `RegisterSchema` (unary), and `FlushNow` (unary, test-only) RPCs on `:9090`. The server is pinned to one table via `MICEWRITER_TABLE` and **rejects cross-table writes** — it validates that each record's `[u16 table_name_len][table_name UTF-8][CBOR bytes]` envelope matches the table it owns. To achieve ultimate zero-copy performance and strict memory boundaries, v2 utilizes **Ahead-Of-Time (AOT) Schema Compilation**. Incoming CBOR records are mapped directly into statically compiled Rust structs tailored to the table's schema using `#[derive(Deserialize)]`, encoded into compact Arrow IPC, written into the active RocksDB column family, and acknowledged in sub-milliseconds.
   * **Why AOT Compilation:** Bypassing dynamic AST parsing entirely eliminates the DOM amplification problem inherent to formats like JSON and CBOR, allowing a 16 MB payload to parse directly into a compact, statically-typed footprint. The engine ships as a **single shared image** containing compiled handlers for every known table; `MICEWRITER_TABLE` selects the active handler at startup via a generated `match` — one branch executes per process lifetime, the rest are dormant compiled-in code. Adding a new table means adding a `schemas/<table>.json` definition file and rebuilding the one image; a `build.rs` codegen step generates the handler code automatically. **Current status:** `schema_codegen.rs` is manually maintained for `load_test_events`; the `build.rs` codegen step has not been implemented yet.
