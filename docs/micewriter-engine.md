...
1. **UDS Listener:** It spins up a raw Tokio `UnixListener` on the shared `/var/run/app/iceberg.sock`, converts each incoming JSON telemetry payload straight into compact Arrow IPC via a pool of `arrow-json` parsers, writes the IPC bytes into RocksDB, and returns microsecond acknowledgments. (There is no CBOR or `serde_json::Value` DOM hop — JSON is parsed directly to typed Arrow arrays against the registered schema.)
2. **Jittered Flush Loop:** A background Tokio task wakes on a jittered interval (~5 minutes, desynchronized across pods) — or immediately once an active column family crosses the size threshold (`flush_size_bytes`, default 128 MB) — to compile each frozen RocksDB column family into Parquet and execute catalog commits with exponential backoff.
   * **Streaming Parquet:** Frozen IPC records are **streamed** to S3 via `iceberg-rust`'s `RollingFileWriter`/`ParquetWriter` over `opendal` multipart upload, instead of buffering a whole file in memory. Working-set memory is bounded to the Parquet **row group** (~8 MiB default), not the batch — so output file size (default 64 MiB target, tunable) is decoupled from the sidecar's RAM envelope.
