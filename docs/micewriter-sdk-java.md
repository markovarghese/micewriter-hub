...
3. **`IcebergStreamTemplate`:** A Spring Bean injected into the application context. It exposes a `.send(pojo)` method that serializes the object into **Apache Arrow IPC RecordBatches** and streams it over the Unix Domain Socket with minimal latency.
   * **Note on Append-Only:** The SDK is designed exclusively for high-throughput, append-only telemetry. Row-level updates or deletes cannot be sent over this SDK and must be handled via separate mechanisms if needed.
